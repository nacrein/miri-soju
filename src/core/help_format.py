...
def usage_embed(command: commands.Command) -> discord.Embed:
    ...
    e.add_field(name="Usage", value=f"`{usage_line(command)}`", inline=False)
    ...
    e.add_field(
        name="Example",
        value=f"`{PREFIX}{example}`" if example else f"`{usage_line(command)}`",
        inline=False,
    )
    return e
