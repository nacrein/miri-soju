...
_recent_text: dict[tuple[int, int], tuple[str, int]] = {}
...
def record_and_check_duplicate(guild_id: int, user_id: int, norm_text: str, threshold: int) -> bool:
    ...
    key = (guild_id, user_id)
    prev, n = _recent_text.get(key, ("", 0))
    n = n + 1 if norm_text and norm_text == prev else 1
    _recent_text[key] = (norm_text, n)
    return n >= threshold
...
async def apply_violation(bot, guild, member, channel, message, violation, cfg, lists) -> None:
    ...
    if await enforcement.is_exempt(bot, guild, member, cfg, lists["roles"], lists["channels"], channel.id):
    ...
    if cfg.log_only:
        await enforcement.log_action(bot, guild, member, violation, action, minutes, strikes, dry_run=True)
        return
    ...
    await enforcement.log_action(bot, guild, member, violation, action, minutes, strikes, dry_run=False)
    return acted
